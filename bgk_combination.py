...
g_beta = 0.00001  # Buoyancy coefficient
nu = 1  # Kinematic viscosity
omega_m = 1 / (3 * nu + 0.5)  # Relaxation rate for flow (BGK)
D = 0.2  # Thermal diffusivity
omega_s = 1 / (3 * D + 0.5)  # Relaxation rate for temperature (BGK)
n_steps = 10000  # Number of time steps
u_max_threshold = 0.1  # Stability threshold for velocity
...
# Collision step for flow (BGK with buoyancy force)
def collision_f(f, F_y):
    ...
    u2 = ux**2 + uy**2
    cu = 3 * (c[:, 0, None, None] * ux + c[:, 1, None, None] * uy)  # Velocity term
    f_eq = t[:, None, None] * rho * (1 + cu + 1.5 * cu**2 - 1.5 * u2)  # Equilibrium distribution
    f_star = f - omega_m * (f - f_eq)  # BGK collision
    # Add buoyancy force
    F_k = t[:, None, None] * 3 * c[:, 1, None, None] * F_y[None, :, :]
    f_star += F_k  # Apply force in distribution space
...
print("Running simulation with temperature advection (BGK)...")
snapshots = []
...
y = np.arange(ny)
X, Y = np.meshgrid(x, y)
contour = ax.contourf(X, Y, snapshots[0][0].T, levels=20, cmap='hot')
...
def update(frame):
    ax.clear()
    contour = ax.contourf(X, Y, snapshots[frame][0].T, levels=20, cmap='viridis')
